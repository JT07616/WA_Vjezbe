        ...
        Dodaj u košaricu
          ...
          <div class="w-full mt-4 max-w-7xl mx-auto bg-slate-800/80 backdrop-blur-sm
            ...
            <div class="flex-1 grid grid-cols-1 sm:grid-cols-3 gap-2">
                <input v-model="ime" placeholder="Ime" class="px-3 py-2 rounded bg-white text-slate-800
            ...
            >
              ...
              <div class="w-full mt-3 text-right font-bold text-orange-400 text-lg">
                Ukupno košarica: {{ ukupno_kosarica }} €
              </div>
...
const ukupno_kosarica = computed(() => {
  return narucene_pizze.value
    .reduce((sum, s) => sum + s.cijena * s.kolicina, 0)
    .toFixed(2)
})
...
async function naruciPizze() {
  ...
  try {
    ...
    if (!ime.value || !adresa.value || !telefon.value) {
      statusPoruka.value = 'Molimo unesite sve podatke za dostavu.'
      statusTip.value = 'error'
      return
    }

    const ciste_stavke = narucene_pizze.value.map((stavka) => ({
      naziv: stavka.naziv,
      kolicina: stavka.kolicina,
      velicina: stavka.velicina,
    }))

    await axios.post(`${URL_express}/narudzbe`, {
      ime: ime.value,
      adresa: adresa.value,
      telefon: telefon.value,
      narucene_pizze: ciste_stavke,
    })

    statusPoruka.value = 'Narudžba je uspješno poslana.'
    ...
    statusPoruka.value =
      error.response?.data?.error || 'Došlo je do greške pri slanju narudžbe.'
    statusTip.value = 'error'
  }
}
